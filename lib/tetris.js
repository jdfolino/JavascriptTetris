...
function Tetris(grid, starting_interval) {
    ...
    this.rotateClockwise = function (piece) {
        return tryMove(piece, GAME_MOVES.ROTATE_CLOCKWISE);
    };

    this.rotateAntiClockwise = function (piece) {
        return tryMove(piece, GAME_MOVES.ROTATE_ANTI_CLOCKWISE);
    };

    this.moveDown = function(piece) {
        return tryMove(piece, GAME_MOVES.DOWN);
    };

    this.moveLeft = function(piece) {
        return tryMove(piece, GAME_MOVES.LEFT);
    };

    this.moveRight = function(piece) {
        return tryMove(piece, GAME_MOVES.RIGHT);
    };
    ...
    function createRandomPiece(grid) {
        var number = new Util().random(0, 6);
        var piece = null;
        switch (number) {
            case 0:
    ...
    function tryMove(piece, command) {
