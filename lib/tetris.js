...
var LongBar = require('./pieces/leftDownwardBar');
...
module.exports = function(grid, starting_interval) {
    ...
    this.tryMove = function (piece, command) {
        ...
        clone[command.move]();
        var validMove = stillOnGrid(clone, 0, 0) && squaresAlreadyOccupied(piece, command.x_transformation, command.y_transformation) == false;
        if (validMove) {
            piece[command.move]();
        }
        return validMove;
